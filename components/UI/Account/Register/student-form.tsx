...
const StudentRegisterForm: React.FC<{ toggleForm: () => void }> = ({ toggleForm }) => {
  const {
    register,
    formState: { errors },
    handleSubmit,
  ...
  const submit: SubmitHandler<UserRegisterInput> = async (data) => {
    setLoading(true);
      ...
      <div>
        ...
        <motion.div variants={fadeToRightVariant} className="mt-8 space-y-2">
          <form onSubmit={handleSubmit(submit)} className="space-y-3" noValidate>
            ...
            <div className="pt-3">
              <button
                className="w-full py-2 rounded-xl bg-deepRed font-medium min-h-[1.2rem] text-white flex items-center justify-center"
                disabled={loading}
              >
                {loading ? (
